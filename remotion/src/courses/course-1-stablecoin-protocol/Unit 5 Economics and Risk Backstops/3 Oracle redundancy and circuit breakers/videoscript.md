# 3 Oracle redundancy and circuit breakers
...
1. **Multi-source Price Checks**
   - Chainlink 作为主要数据源
   - Uniswap V3 TWAP 作为备用
   - 价格偏差阈值验证
...
contract OracleManager {
    uint256 private constant MAX_DEVIATION = 5e16; // 5%
    uint32 private constant TWAP_PERIOD = 1800; // 30 min
    ...
    function _getTWAPPrice(address token) internal view returns (uint256) {
        IUniswapV3Pool pool = IUniswapV3Pool(tokenToPool[token]);
...
- Time-Weighted Average Price 原理
- `observe()` 函数使用
- Tick 到价格的转换
- 流动性深度考量
...
- 不同网络的 Uniswap V3 部署地址
- Pool fee tier 选择 (0.05%, 0.3%, 1%)
- 冷启动时的价格初始化

---

*本章节将集成 Uniswap V3 作为 Chainlink 的补充验证，提高价格数据的可靠性*
