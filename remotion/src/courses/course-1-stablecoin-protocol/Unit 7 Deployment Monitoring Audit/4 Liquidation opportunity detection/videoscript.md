# 4 Liquidation opportunity detection

> **🎬 待录制 (Coming Soon)**

## 课程大纲

本节将构建清算机器人，识别并执行盈利清算：
...
2. **MEV Protection**
   - Flashbots 集成基础
   - Private transaction 提交
   - Front-running 防护

3. **Liquidation Bot Architecture**
   - 目标用户扫描策略
   - 优先级队列设计
   - 并发执行优化
...
class LiquidationBot {
  async scanOpportunities(): Promise<LiquidationOpportunity[]> {
    const users = await this.getAllUsers();
    const opportunities: LiquidationOpportunity[] = [];
    
    for (const user of users) {
      const healthFactor = await this.getHealthFactor(user);
      
      if (healthFactor >= parseEther('1')) continue; // 健康的
      
      const { debt, collateral } = await this.getUserPosition(user);
      const estimatedProfit = this.calculateProfit(debt, collateral);
  ...
  calculateProfit(
    ...
    // 清算奖励: 10% bonus
    const collateralToReceive = (debt * 110n) / 100n;
    
    // 估算 Gas 成本 (假设 300k gas @ 20 gwei)
    const gasPrice = parseGwei('20');
    const gasLimit = 300000n;
    const gasCost = gasPrice * gasLimit;
    
    // 净利润 = 抵押品价值 - 债务 - Gas
    return collateralToReceive - debt - gasCost;
  }
  
  async executeLiquidation(opp: LiquidationOpportunity) {
    console.log(`🎯 Liquidating ${opp.user}`);
    console.log(`Expected profit: ${formatEther(opp.estimatedProfit)} ETH`);
    ...
    try {
      ...
      await this.client.simulateContract({
        ...
        args: [opp.user, WETH_ADDRESS, opp.debtToCover],
      ...
      if (receipt.status === 'success') {
        await this.recordSuccess(opp, receipt);
      }
    } catch (error) {
      console.error('❌ Liquidation failed:', error);
      await this.recordFailure(opp, error);
    }
  }
}
...
setInterval(async () => {
  ...
  if (opportunities.length > 0) {
    ...
    for (const opp of opportunities.slice(0, 3)) {
      await bot.executeLiquidation(opp);
    }
  }
}, 15000); // 每 15 秒扫描
```
