...
本节将使用 TypeScript + Viem 构建链下监控系统：

### 核心内容

1. **Event Monitoring**
   - Viem `watchContractEvent` 使用
   - 事件过滤与解析
   - 数据库存储 (PostgreSQL)

2. **Health Factor Tracking**
   - 批量查询用户健康因子
   - 低于阈值的告警
   - Telegram/Discord 通知
...
const unwatch = client.watchContractEvent({
  ...
  onLogs: (logs) => {
    logs.forEach(async (log) => {
      const { user, liquidator, debtCovered, collateralLiquidated } = log.args;
      
      console.log(`🚨 Liquidation Alert!`);
      console.log(`User: ${user}`);
      console.log(`Debt: ${formatEther(debtCovered)} DSC`);
...
setInterval(async () => {
  ...
  for (const user of users) {
    ...
    if (healthFactor < parseEther('1.1')) {
      await sendAlert({
        type: 'health_warning',
        user: user.address,
...
const ALERT_RULES = {
  healthFactor: {
    critical: 1.05,  // < 1.05 立即通知
    warning: 1.2     // < 1.2 警告
  },
  protocolHealth: {
    critical: 1.1,   // 协议整体不健康
    warning: 1.3
  },
  oracleStale: {
    maxDelay: 3600   // 1 小时未更新
  }
};
```
