...
本节将实现 Stability Pool 来处理极端市场下的坏账：

### 核心内容

1. **Liquity Approach**
   - 用户存入 DSC 赚取清算折扣
   - Product-Sum 算法高效分配奖励
   - P (product) 和 S (sum) 变量追踪
...
3. **Liquidation Flow Integration**
   - 优先使用 Stability Pool
   - Pool 不足时回退到常规清算
   - 坏账处理的最后防线
...
清算发生时:
- P = P * (totalDeposits - debtCovered) / totalDeposits
- S = S + collateralGain / totalDeposits

用户当前存款:
currentDeposit = initialDeposit * (P / P_snapshot)

用户抵押品收益:
collateralGain = currentDeposit * (S - S_snapshot) / P_snapshot
```
...
contract StabilityPool {
    ...
    uint256 public S; // Per ETH
    
    mapping(address => uint256) public deposits;
    mapping(address => Snapshots) public snapshots;
    ...
    function liquidateWithPool(
        ...
        uint256 collateralToDistribute = _getCollateralFromDebt(debtToCover);
        
        // 更新 P 和 S
        P = P * (totalDeposits - debtToCover) / totalDeposits;
        S = S + (collateralToDistribute * 1e18) / totalDeposits;
        
        // 销毁 DSC
        dsc.burn(debtToCover);
    }
}
```
...
*本章节基于 Liquity Protocol 和代码库中的 `src/StabilityPool.sol` 实现*
